"""
Vietnamese Text-to-Speech Module using XTTS
"""
import os
import string
import torch
...
def reset_tts():
    """Reset the TTS model in case of errors"""
    try:
...
def text_to_speech(text, lang="vi", normalize=True):
    ...
    try:
        ...
        if not tts_engine.is_initialized:
            if not initialize():
                raise Exception("TTS engine not initialized and initialization failed")
        ...
        logger.error(f"Error in text_to_speech function: {e}")
        logger.error(traceback.format_exc())
        raise

def is_initialized():
    """Check if the TTS engine is initialized"""
    return tts_engine.is_initialized
...
if __name__ != '__main__':
    ...
    # You can choose to initialize immediately or defer until first use
    # initialize()